...
// PASSTHROUGH FIX: copy of Reallusion's RL_HairShader_1st_Pass, with the single
// change of removing "keepalpha" on the #pragma surface line below. That makes the
// forward pass write alpha = 1 into the Quest eye buffer, so passthrough no longer
// bleeds through the hair. All color/lighting is identical to the original CC hair.
Shader "Quest/HairOpaque"
{
	Properties
	{
	...
	SubShader
	{
		...
		// Meta Depth API (Quest 3): lets real-world geometry occlude the avatar.
		...
		inline half4 LightingStandardCustomLighting( inout SurfaceOutputCustomLightingCustom s, half3 viewDir, UnityGI gi )
		{
			...
			c.a = temp_output_521_0;
				...
				// Meta Depth API: discard fragments behind real-world geometry (hard occlusion).
				float envOcclusion = CalculateEnvironmentDepthOcclusion( i.worldPos, 0.0 );
				clip( envOcclusion - 0.5 );
			#endif
			return c;
		}
		...
		ENDCG
		CGPROGRAM
		#pragma surface surf StandardCustomLighting fullforwardshadows nometa
